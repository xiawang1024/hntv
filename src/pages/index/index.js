...
export default class Index extends Component {
  ...
  render() {
    ...
    return ( <
      ...
      circular autoplay duration = '300' > {
        this.state.swipeList.map((item, index) => {
          return ( <
            SwiperItem onClick = {
              this.goToBody.bind(this, item.id)
            } >
            <
            Image key = {
              item.id.toString()
      ...
      View >
      <
      Image className = 'border-radius'
      src = {
        require('./border-radius.png')
      }
      style = {
        {
          width: '100%'
      ...
      View className = 'model-wrap' > {
        modelData.map((item) => {
          return ( <
            View className = 'item'
            onClick = {
              this.onClickHandler.bind(this, item.id)
            }
            key = {
              item.id.toString()
            } >
      ...
      /View> { /* <View className='line-h' /> */ } <
      ScrollView className = 'scrollview'
      scrollX lowerThreshold = '20'
      upperThreshold = '20' > {
